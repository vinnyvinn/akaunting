...
class Invoices
{
    ...
    public function compose(View $view)
    {
        ...
        if ($view->getName() == 'sales.invoices.edit') {
            $invoice = $view->getData()['invoice'];

            $custom_field_values = FieldValue::record($invoice->id, get_class($invoice))->get();
            $custom_field_item_values = FieldValue::record($invoice->id, get_class($invoice))->get();
        }
        ...
        if ($custom_fields) {
            foreach ($custom_fields as $custom_field) {
                ...
                if (strpos($custom_field->fieldLocation->sort_order, 'td') !== false) {
                    ...
                    $table_name_view = 'custom-fields::invoices.item_th';
                    $table_item_view = 'custom-fields::invoices.item_td_input';

                    $view->getFactory()->startPush($table_name, view($table_name_view, compact($compact)));

                    if (($view->getName() == 'sales.invoices.create' || $view->getName() == 'sales.invoices.edit') && in_array($table_item, $skipes)) {
                        continue;
                    }
                    ...
                    if ($view->getName() == 'sales.invoices.item') {
                        continue;
                    }
