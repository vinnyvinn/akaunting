            ...
            <table class="table align-items-center table-flush">
                <thead class="thead-light">
                    <tr class="row table-head-line">
                        ...
                        <th class="col-xs-4 col-md-4 text-left">{{ trans_choice('projects::general.like', 2) }}</th>
                        <th class="col-xs-4 col-md-4 text-right">{{ trans_choice('projects::general.comment', 2) }}</th>
                    </tr>
                </thead>
                <tbody>
                    @if ($discussions->count())
                        @foreach($discussions as $item)
                            <tr class="row border-top-1">
                                ...
                                <td class="col-xs-4 col-md-4 text-left">{{ $item->total_like }}</td>
                                <td class="col-xs-4 col-md-4 text-right">{{ $item->total_comment }}</td>
                            </tr>
                        @endforeach
                    @else
