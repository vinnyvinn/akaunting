                    ...
                    @if ($items->count())
                        @foreach($items as $item)
                            <tr class="row border-top-1 tr-py">
                                <td class="col-xs-6 col-md-6 text-left">{{ $item->get('item_name') }}</td>
                                <td class="col-xs-6 col-md-6 text-right">{{ money($item->get('amount'), setting('default.currency', 'USD'), true) }}</td>
                            </tr>
                        @endforeach
                    @else
