...
class Company extends Model
{
    ...
    {
        return $this->hasMany('Modules\Crm\Models\CompanyContact', 'crm_company_id', 'id');
    }
    ...
    {
        return $this->hasMany('Modules\Crm\Models\Deal', 'crm_company_id', 'id');
    }
    ...
    {
        return $query->join('contacts', 'crm_companies.contact_id', '=', 'contacts.id')->where('contacts.enabled', 1)->select('crm_companies.*');
    }

    public function scopeDisabled($query)
    {
        return $query->join('contacts', 'crm_companies.contact_id', '=', 'contacts.id')->where('contacts.enabled', 0)->select('crm_companies.*');
    }
}
